...
#define HYDRO_SPH_GRAVITY 0
#define HYDRO_SPH_PRESSURE 1
...
static void reb_calculate_acceleration_for_sph_particle(const struct reb_simulation* const r, const int pt, const struct reb_ghostbox gb, int mode);
...
/**
  ...
  * @param gb Ghostbox plus position of the particle (precalculated). 
  */
static void reb_calculate_gravity_for_sph_particle_from_cell(const struct reb_simulation* const r, const int pt, const struct reb_treecell *node, const struct reb_ghostbox gb);

static void reb_calculate_pressure_for_sph_particle_from_cell(const struct reb_simulation* const r, const int pt, const struct reb_treecell *node, const struct reb_ghostbox gb);
  
static void reb_calculate_acceleration_for_sph_particle(const struct reb_simulation* const r, const int pt, const struct reb_ghostbox gb, int mode) {
	for(int i=0;i<r->root_n;i++){
		struct reb_treecell* node = r->tree_root[i];
			if (node!=NULL){
				if (mode == HYDRO_SPH_GRAVITY) reb_calculate_gravity_for_sph_particle_from_cell(r, pt, node, gb);
				if (mode == HYDRO_SPH_PRESSURE) reb_calculate_pressure_for_sph_particle_from_cell(r, pt, node, gb);
			} else {
				printf("Node is NULL\n");
			}
	}
...
static void reb_calculate_gravity_for_sph_particle_from_cell(const struct reb_simulation* r, const int pt, const struct reb_treecell *node, const struct reb_ghostbox gb) {
	const double G = r->G;
	struct reb_particle* const particles = r->particles;
	// double softening2;
	...
	if ( node->pt < 0 ) { // Not a leaf
		...
		if ( (node->w*node->w > r->opening_angle2*r2) || ( MAX(MAX(dxij*dxij, dyij*dyij), dzij*dzij) < thesdist*thesdist ) ){
			for (int o=0; o<8; o++) {
				if (node->oct[o] != NULL) {
					reb_calculate_gravity_for_sph_particle_from_cell(r, pt, node->oct[o], gb);
				}
			}
		} else {
			double _r = sqrt(r2);
...
static void reb_calculate_pressure_for_sph_particle_from_cell(const struct reb_simulation* r, const int pt, const struct reb_treecell *node, const struct reb_ghostbox gb) {
	// const double G = r->G;
	struct reb_particle* const particles = r->particles;
	// double softening2;
	const double dx = gb.shiftx - node->mx;
	...
	if ( node->pt < 0 ) { // Not a leaf
		...
		if ( MAX(MAX(dx*dx, dy*dy), dz*dz) < thesdist*thesdist ){
			for (int o=0; o<8; o++) {
				if (node->oct[o] != NULL) {
					reb_calculate_pressure_for_sph_particle_from_cell(r, pt, node->oct[o], gb);
				}
			}
		}
	} else { // It's a leaf node
