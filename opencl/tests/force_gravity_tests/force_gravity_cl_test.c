...
void force_gravity_cl_test(int num_bodies, int num_threads_tree_kernel, int num_threads_tree_gravity_kernel, int num_threads_tree_sort_kernel, int num_threads_force_gravity_kernel)
{
  ...
  if (error != CL_SUCCESS) {
    fprintf(stderr,"clSetKernelArg ERROR (force_gravity_kernel/bottom_node_buffer): %s\n",cl_host_tools_get_error_string(error));
    exit(EXIT_FAILURE);
  }
...
/* #endif */
  ...
  /* error = clSetKernelArg(force_gravity_kernel, 25, sizeof(cl_mem), &error_buffer); */
  /* if (error != CL_SUCCESS) { */
  /*   fprintf(stderr, "clSetKernelArg Error (force_gravity_kernel/error_buffer): %s\n",cl_host_tools_get_error_string(error)); */
  /*   exit(EXIT_FAILURE); */
  /* } */
