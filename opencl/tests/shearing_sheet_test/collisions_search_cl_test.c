...
/*
  ...
  ****Add ghostboxes to gravity tree kernel
  ****Add global->local->private time storage to CL_GRAVITY AND CL_COLLISION tree kernels
  ****Kahan summation
  ****OpenGL
  ****Compaction (maybe not needed)
  ****add G etc units to force gravity kernel
  ****either make a new tree gravity with count or make count kernel
  */

void collisions_search_cl_test(int num_bodies, int num_threads_tree_kernel, int num_threads_tree_gravity_kernel, int num_threads_tree_sort_kernel, int num_threads_force_gravity_kernel, int num_threads_collisions_search_kernel, int num_threads_collisions_resolve_kernel, int num_threads_boundaries_kernel, int num_threads_integrator_kernel)
{
  cl_device_id device;
  cl_context context;
  cl_program program;
  ...
  cl_float OMEGAZ_host = -1;
  cl_float sindt_host;
  cl_float tandt_host;
  cl_float sindtz_host;
  cl_float tandtz_host;
  cl_float dt_host = 1e-3*2.*M_PI/OMEGA_host;
  ...
  error = clSetKernelArg(force_gravity_kernel, 17, sizeof(cl_int)*wave_fronts_in_force_gravity_kernel, NULL);
  if (error != CL_SUCCESS) {
  ...
  error = clSetKernelArg(force_gravity_kernel, 20, sizeof(cl_float)*wave_fronts_in_force_gravity_kernel*MAX_DEPTH, NULL);
  if (error != CL_SUCCESS) {
  ...
  error = clSetKernelArg(force_gravity_kernel, 25, sizeof(cl_int)*local_size_force_gravity_kernel, NULL);			 
  if (error != CL_SUCCESS) {
